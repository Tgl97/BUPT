...
struct frame
{
	...
	unsigned char ack ;            //帧的ack号
	unsigned char seq ;            //帧的序列号
	unsigned char info[PKT_LEN] ;  //网络层分组
...
static void send_data_frame(unsigned char fk, unsigned char frame_nr, unsigned char frame_expected, unsigned char buffer[MAX_SEQ + 1][PKT_LEN])
//发送数据帧、ack、nak
{
    struct frame s ;
